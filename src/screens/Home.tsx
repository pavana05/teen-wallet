import { Bell, Home as HomeIcon, ScanLine, ShoppingBag, CreditCard, ArrowUpRight, Building2, Wallet, History, Plus, ArrowDownLeft } from "lucide-react";
    ...
    <div className="flex-1 flex flex-col tw-slide-up pb-28">
      ...
      <div className="px-6 mt-8">
        <div className="grid grid-cols-4 gap-3">
          {[
            { icon: ScanLine, label: "Scan & Pay", primary: true },
      ...
      <div className="px-6 mt-8 flex-1">
        ...
        {txns.length === 0 ? (
          <div className="glass rounded-3xl p-8 text-center">
            ...
            <p className="text-sm text-white">No transactions yet</p>
            <p className="text-xs text-muted-foreground mt-1">Make your first payment to see it here.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {txns.map((t) => (
              <div key={t.id} className="glass rounded-2xl p-4 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-primary/20 text-primary flex items-center justify-center font-semibold text-xs">
                ...
                <p className="text-sm num-mono text-destructive">−₹{Number(t.amount).toFixed(2)}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-[400px] z-50">
        <div className="flex items-center gap-2">
          ...
          <button className="w-14 h-14 rounded-full bg-white text-black flex items-center justify-center lime-glow shadow-lg">
